...
# limit from the shared/global quota_service.py pool every AI feature draws
# from. Same in-memory, single-Render-instance, day-bucket-reset shape as
# quota_service.py's own state (see that module's _get_state for the
# identical idiom), just keyed by user_id instead of (key_alias, model) —
# nothing else in this backend currently tracks anything per-user-per-day.
...
def record_turn(user_id: str) -> None:
    """Call exactly once per chat turn that actually reaches Gemini — mirrors
    quota_service.record_gemini_call's "only count real attempts" rule.
